...
sub maketables
{
	...
	# shuffle the order values for each variable, so that a different set of pairings is produced each time Allpairs is run.
	foreach my $listname (@{$labels{$tablename}})
	{
		my %order = ();
		foreach(0..@{$lists{$listname}}-1)
		{
			$order{$_} = rand(1);
		}		
		@{$lists{$listname}} = (map $lists{$listname}->[$_], sort {$order{$a} <=> $order{$b}} keys %order);
	}
